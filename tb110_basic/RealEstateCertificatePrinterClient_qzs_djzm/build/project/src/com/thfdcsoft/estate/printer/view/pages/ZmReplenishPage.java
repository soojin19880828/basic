...
private static Text remaining = new Text(MainView.remaining);   //0
	...
	public static void setRemaining(String n) {
		...
		req.setTerminalId(Constant.TerminalInfo.DJZM_TYPE);
	...
	public static void getRemaining() {                           //数据库证书数量
		...
		try {
			...
			if (rspn.isResult()) {
				Platform.runLater(() -> {
					ZmReplenishPage.remaining.setText(rspn.getRespJson());
	...
	public static Pane buildPage() { 
		...
		Text label  = new Text("证明总量不能超过150：");
	...
	//	replPane.add(remaining, 3, 0, 2, 1);// 第一行，占用第四第五列
		...
		// 剩余空白证明
		Label remainingLabel = new Label("剩余空白证明：");
		...
		System.out.println("获取总的证明数量:"+sum);
	...
	//			
				if(Integer.parseInt(remaining.getText())>150) {
					remaining.setText("150");
				}
